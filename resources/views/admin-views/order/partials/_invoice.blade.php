            ...
            <div class="order-info-details">
                <div class="row mt-3">
                    ...
                    <div class="col-12">
                        ...
                        <h5><u>{{translate('messages.info')}} {{translate('messages.sender')}}</u></h5>
                        <div>
                            {{translate('messages.name')}} {{translate('messages.sender')}} : {{isset($address)?$address['contact_person_name']:$order->address['f_name'].' '.$order->customer['l_name']}}
                        </div>
                        <div>
                        ...
                        <div>
                            {{translate('messages.name')}} {{translate('messages.receiver')}} : {{isset($address)?$address['contact_person_name']:$order->address['f_name'].' '.$order->customer['l_name']}}
                        </div>
                        <div>
                            {{translate('messages.phone')}} : {{isset($address)?$address['contact_person_number']:$order->customer['phone']}}
                        </div>
                                ...
                                <tr>
                                    <td class="text-break">
                                        ...
                                        @if(count(json_decode($detail['variation'],true))>0)
                                            <strong><u>{{translate('messages.in_this')}} : </u></strong>
                                            @foreach(json_decode($detail['variation'],true)[0] as $key1 =>$variation)
                                                @php($trung=json_decode($detail['choice_options'],true))
                                                {{$trung}}
                                                @if ($key1 != 'stock')
                                                    <div class="font-size-sm text-body">
                                                        <span>{{$key1=='type'?'Loại':($key1=='price'?'Giá':$key1)}} :  </span>
