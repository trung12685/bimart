            ...
            <div class="row align-items-center">
                <div class="col-xl-10 col-md-9 col-sm-8 mb-3 mb-sm-0 {{$parcel_order ? 'mb-2':''}}">
                    <h1 class="page-header-title text-capitalize m-0">
                        ...
                        <span>
                            @if ($parcel_order) {{translate('messages.orders')}} {{translate('messages.parcel')}} @else {{translate('messages.orders')}} {{translate(str_replace('_',' ',$status))}} @endif <span class="badge badge-soft-dark ml-2">{{$total}}</span>
                        </span>
                    </h1>
                </div>
                ...
                <div class="search--button-wrapper justify-content-end">
                    ...
                    <div class="hs-unfold mr-2">
                        ...
                        <div id="usersExportDropdown"
                            ...
                            <span class="dropdown-header">{{translate('messages.options')}} {{translate('messages.download')}}</span>
                        ...
                        <div id="showHideDropdown"
                            ...
                            <div class="card card-sm">
                                <div class="card-body">
                                    ...
                                    {{-- <div class="d-flex justify-content-between align-items-center mb-3">
                                        <span class="mr-2 text-capitalize">{{translate('messages.status')}} {{translate('messages.payment')}}</span>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <span class="mr-2">{{translate('messages.status')}} {{translate('messages.order')}}</span>
                    ...
                    @foreach($orders as $key=>$order)
                        ...
                        <tr class="status-{{$order['order_status']}} class-all">
                            ...
                            <td>
                                ...
                                @else
                                    <label class="badge badge-danger">{{translate('messages.data')}} {{translate('messages.customer')}} {{translate('messages.invalid')}}</label>
                                @endif
                            </td>
                            @if ($parcel_order)
                            ...
                            <td>
                                @if ($parcel_order)
                                    <div>{{Str::limit($order->parcel_category?$order->parcel_category->name:translate('messages.not_found'),200,'...')}}</div>
                                @elseif ($order->store)
                                    <div><a  class="text--title" href="{{route('admin.vendor.view', $order->store_id)}}" alt="view store">{{Str::limit($order->store?$order->store->name:translate('messages.store deleted!'),200,'...')}}</a></div>
                                @else
                                    <div>{{Str::limit(translate('messages.not_found'),200,'...')}}</div>
                                @endif
                            </td>
                            <td>
        ...
        <div id="datatableFilterSidebar" class="hs-unfold-content_ sidebar sidebar-bordered sidebar-box-shadow initial-hidden">
            <div class="card card-lg sidebar-card sidebar-footer-fixed">
                <div class="card-header">
                    <h4 class="card-header-title">{{translate('messages.filter')}} {{translate('messages.order')}}</h4>
                ...
                <form class="card-body sidebar-body sidebar-scrollbar" action="{{route('admin.order.filter')}}" method="POST" id="order_filter_form">
                    ...
                    <hr class="my-4">
                    @if($status == 'all')
                    <small class="text-cap mb-3">{{translate('messages.status')}} {{translate('messages.order')}}</small>
                    ...
                    @if (!$parcel_order)
                        <hr class="my-4">
                        <small class="text-cap mb-3">{{translate('messages.type')}} {{translate('messages.order')}}</small>
                        <div class="custom-control custom-radio mb-2">
