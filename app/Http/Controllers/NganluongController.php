...

{
    ...
    public function __construct()
    {
        ...
        $config = Helpers::get_business_settings('nganluong');
        // You can import it from your Database
        $url_api = $config['url_api']; // bKash Merchant API APP KEY
        $receiver_email = $config['receiver_email']; // bKash Merchant API APP SECRET
        $merchant_id = $config['merchant_id']; // bKash Merchant API USERNAME
        $merchant_pass = $config['merchant_pass']; // bKash Merchant API PASSWORD
    ...
    public function payment(Request $request)
    {
        ...
        $user = $order->customer;
        // echo($user->f_name);
        $nlcheckout= new NL_CheckOutV3($this->merchant_id,$this->merchant_pass,$this->receiver_email,$this->url_api);
        $total_amount="$value";
		...
		$chiadulieu = $request['paymethod'];
        $dulieu = explode("-",$chiadulieu);
        $payment_method =$dulieu[0];
	    $bank_code = @$dulieu[1];
        $order_code ="macode_".time();
        $payment_type ='';
        $discount_amount =0;
        $order_description='Bimart Order';
        $tax_amount=0;
        $fee_shipping=0;
        $return_url =url('/') . "/nganluong-response";
        $cancel_url =url('/') . "/nganluong-response" ;
        $buyer_fullname =$user->f_name." ".$user->l_name;
	    ...
	    if ($nl_result->error_code =='00'){
            ...
            header('Location:' . (string)$nl_result->checkout_url); /* Redirect browser */
            exit();
        }
        ...
        if($nl_result){
        	...
        	if($nl_errorcode == '00') {
        		if($nl_transaction_status == '00') {
        			$order->payment_method = 'Nganluong';
    ...
    {   
        ...
        // $order->callback . '&status=fail'
        // //return view('nganluong-index', compact('data', 'order'));
