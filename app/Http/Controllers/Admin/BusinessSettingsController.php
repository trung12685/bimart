...
class BusinessSettingsController extends Controller
{
    ...
    public function payment_update(Request $request, $name)
    {
        ...
        if ($name == 'cash_on_delivery') {
            ...
            DB::table('business_settings')->updateOrInsert(['key' => 'vnpay'], [
                'value' => json_encode([
                    'status' => $request['status'],
                    'vnp_TmnCode' => $request['vnp_TmnCode'],
                    'vnp_HashSecret' => $request['vnp_HashSecret'],
                    'vnp_Url' => $request['vnp_Url'],
                    'vnp_apiUrl' => $request['vnp_apiUrl'],
                    
                ]),
                'updated_at' => now()
            ]);
        } elseif ($name == 'nganluong') {
            DB::table('business_settings')->updateOrInsert(['key' => 'nganluong'], [
                'value' => json_encode([
                    'status' => $request['status'],
                    'url_api' => $request['url_api'],
                    'receiver_email' => $request['receiver_email'],
                    'merchant_id' => $request['merchant_id'],
                    'merchant_pass' => $request['merchant_pass'],
